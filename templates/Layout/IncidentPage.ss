<div class="row spread contentsec">
		<% loop MainStatement %>
		<div class="col-md-8 col-sm-8 col-sx-12">
		<h3>$PRPlace 
		</h3>
		$Content
		<br /><br />
		</div>
		<% end_loop %>
		
		<% loop MainStatement %>
		<% if showReleases %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			<h3>Press Releases</h3><span class='rsswidget'><a href="/flight/$FltNum/rss/"><img src="/themes/skywest/images/feed_24x24.png"/ alt="rss"></a></span>
			<% loop Top.DarkReleases %><br />
			<a href="$DarkRelease.URL" target="_blank" class="pdfBoxLink"><div class="prDate">$Date.format(m/d/Y) </div>
			<div class="pdfSummary">$Title</div></a>
			<br />
			<% end_loop %>
		</div>
		<% else %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			&nbsp;
		</div>
		<% end_if %>
		<% end_loop %>
		
		<% loop MainStatement %>
		<% if showResources %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			<h3>Resources</h3>
			<% loop Top.DarkResources %>
			<div class="{$EvenOdd}"><a href="$DarkResource.URL" target="_blank">$Title</a></div>
			<% end_loop %>
		</div>
		<% else %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			&nbsp;
		</div>
		<% end_if %>
		<% end_loop %>
		
		<% loop MainStatement %>
		<% if showPartners %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			<% loop Top.DarkPartner %>
			<h3>Partner Info</h3>
			<% if DarkLogo %><img class="left" src="$DarkLogo.URL" alt="Partner Logo"/><% end_if %>
			$PartnerContent
			<p><a href="$DarkPartnerLink" target="_blank">Click here</a> to access {$Title}' site.</p>
			<% end_loop %>
		</div>
		<% else %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			&nbsp;
		</div>
		<% end_if %>
		<% end_loop %>
		
		<% loop MainStatement %>
		<% if showContacts %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			<p><strong>Media Inquiries</strong>
			<br />
			$MediaInq</p>
			<% if showFamilyFone %>
			<p><strong>Information for Family Members</strong></p>
			<span style="margin-top:-12px;display:block;">$FamilyInq</span>
			<p><strong>Family Members Call:</strong><br/>$Familyfone</p> <% end_if %>
		</div>		<% else %>
		
		<% end_if %>
		<% end_loop %>
		
		<% loop MainStatement %>
		<% if showBriefing %>
		<div class="col-md-4 col-sm-4 col-sx-12">
			<h3>Media Briefing</h3>
			<div class="briefingLeft">Location:</div> <div class="briefingRight">
				$MediaLoc
			</div>
			<div class="briefingLeft">Address:</div> <div class="briefingRight">
				$MediaAddress
			</div>
			<div class="briefingLeft">Date:</div> <div class="briefingRight">$MediaDate.Format(F d), $MediaDate.Format(Y)</div>
			<div class="briefingLeft">Time:</div> <div class="briefingRight">$MediaTime</div>
			<div class="briefingLeft">Speakers:</div> <div class="briefingRight">$MediaSpeak</div>
		</div>
		<% else %>
		
		<% end_if %>
		<% end_loop %>		
		
		<div class='ispostupdate' style='float:right;color:#00529B;'>
		<% if isUpdated %>
			<i style="font-size:12px; color:#333;">Updated</i>
		<% end_if %> 
		<% loop MainStatement %>
			<i style="font-size:12px;">$LastEdited.format(m/d/Y H:i) MT</i>
		<% end_loop %>
		</div>
</div>
