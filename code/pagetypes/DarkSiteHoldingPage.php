...
class DarkSiteHoldingPage extends Page {
	...
	*/
	public static function redirect_to_dark() {
		...
		if($ds) {	/*********** DARK SITE IS ACTIVE **************/
			//if ($darkPage->exists() && $darkPage instanceof DarkSiteHoldingPage && $darkPage->getExistsOnLive()) {
				...
				if(Director::get_current_page()->ClassName == HomePage::class) {	// only redirect the home page --
					return Controller::curr()->redirect('/flight/');	//$darkPage->Link() . $ds->FltNum
	...
	public static function FltNum($fltNum = '') {
		//Debug::show('poop');
		if($fltNum) {
			$f = "`FltNum` = '$fltNum'";
	...
	*/
	...
	public function checkDarkActive() {
